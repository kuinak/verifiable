...
Associated objects don't show up until they're verified:
  ...
  user.phone_numbers == [] # true

Get unverified objects like this:

  user.unverified_phone_numbers == [phone_number] # true

Get the verification code for an association:
...
Once verified, the associated objects show up:
  ...
  user.phone_numbers == [phone_number] # true
